...
export default {
  ...
  mounted() {
    ...
    if (this.$route.query.code) {
      this.kcReportedSelect.code = this.$route.query.code;
      this.getKcReportedLists();
    }
  },
  methods: {
    ...
    getKcReportedLists(val) {
      ...
      getKcReportedList(
        this.kcReportedSelect,
        this.kcReportedPage.pageNum,
        this.kcReportedPage.pageSize
      ).then((resp) => {
        if (resp.code != 200) {
          // 失败
...
<template>
  <div>
    ...
    <div class="condition">
      ...
      <el-button type="primary" @click="addReportedVisible = true"
        >添加</el-button
      >
          ...
          <template #default="{ row }">
            ...
            <el-dropdown>
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item @click.native="print(row.code)"
                  >打印</el-dropdown-item
                >
                <el-dropdown-item @click.native="deleteReported(row)"
                  >删除</el-dropdown-item
                >
                <el-dropdown-item
                  @click.native="checkReporteds(row)"
                  v-if="row.approvalStatus == 0"
                  >审批</el-dropdown-item
                >
              </el-dropdown-menu>
    ...
    <div>
      <el-dialog
        title="添加报损"
        :visible.sync="addReportedVisible"
        width="1500px"
      >
        <AddReported
          @handleAddSuccess="
            addReportedVisible = false;
            getKcReportedLists(1);
          "
    ...
    <div>
      <el-dialog
        title="修改报损"
        :visible.sync="updateReportedVisible"
        width="1500px"
      >
        <UpdateReported
          :row-data="updateReported"
          @handleAddSuccess="
            updateReportedVisible = false;
            getKcReportedLists(1);
          "
    ...
    <div>
      <el-dialog
        title="审批报损"
        :visible.sync="checkReportedVisible"
        width="1500px"
      >
        <CheckReported
          :row-data="updateReported"
    ...
    <div>
      <el-dialog
        title="详情报损"
        :visible.sync="detailsReportedVisible"
        width="1500px"
      >
        <DetailsReported
          :row-data="updateReported"
